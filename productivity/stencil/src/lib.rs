...
pub struct Stencil {
    /// How many bytes are necessary to enclose the width of the stencil.
    pub span: usize,
...
impl Stencil {
    ...
    pub fn filled_rectangle(&mut self, upper_left: (usize, usize), lower_right: (usize, usize), pattern: &[u8; 8]) {
        ...
        for y in 0..height {
            self.horizontal_line((left, top + y), right, pattern[y & 7]);
        }
    }

    /// Draw a horizontal line with the given pattern.
    pub fn horizontal_line(&mut self, left: (usize, usize), right: usize, pattern: u8) {
        let (left, top) = left;
        ...
        let right = right - 1;
        let left_byte = self.span * top + (left >> 3);
        let right_byte = self.span * top + (right >> 3);
        let left_mask = LEFT_MASKS[left & 7];
        let right_mask = RIGHT_MASKS[right & 7];
